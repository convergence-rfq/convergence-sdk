...
test('[loadConvergenceModule] it can load a Metadata model into an RFQ', async (t: Test) => {
  const cvg = await convergence();
  const originalRfq = await createRfq(cvg, {
    name: 'On-chain Name',
    json: { name: 'Json Name' },
  });
  const metadata = await asMetadata(cvg, originalRfq);
  const rfq = await cvg.rfqs().load({ metadata });
...
const asMetadata = async (
  ...
  const metadataAccount = toMetadataAccount(
    await convergence.rpc().getAccount(rfq.metadataAddress)
  );
