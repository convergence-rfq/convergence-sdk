...
test("[nftModule] it can load a Metadata model into an NFT", async (t: Test) => {
  ...
  const mx = await convergenceRfq();
  const originalNft = await createRfq(mx, {
    name: "On-chain Name",
    json: { name: "Json Name" },
  });
...
const asMetadata = async (
  convergenceRfq: ConvergenceRfq,
  nftOrSft: Nft | Sft
): Promise<Metadata> => {
  const metadataAccount = toMetadataAccount(
    await convergenceRfq.rpc().getAccount(nftOrSft.metadataAddress)
  );
