...
export const createSftOperationHandler: OperationHandler<CreateSftOperation> = {
  handle: async (
    ...
    const sft = await convergence.rfqs().findByMint(
      {
...
export const createSftBuilder = async (
  ...
  const metadataPda = convergence.rfqs().pdas().metadata({
    mint: mintAddress,
    programs,
  });
  ...
  const verifyAdditionalCreatorInstructions = creatorsInput
    ...
    .map((creator) => {
      return convergence.rfqs().builders().verifyCreator(
        {
          mintAddress,
...
> => {
  ...
  if (tokenAddress && params.tokenAmount) {
    builder.add(
      await convergence.tokens().builders().mint(
        {
          mintAddress,
