 ...
 * @category Inputs
 */
export type CreateAndAddLegsToRfqBuilderParams = CreateAndAddLegsToRfqInput;
/**
 * Prepares for settlement
 *
 * ```ts
 ...
 *   .rfqs()
 *   .builders()
 *   .createAndAddLegsToRfq({ address });
 * ```
 *
 * @group Transaction Builders
 * @category Constructors
 */
export const createAndAddLegsToRfqBuilder = async (
  convergence: Convergence,
  params: CreateAndAddLegsToRfqBuilderParams,
  ...
  console.log('tx size: ' + txSize.toString());

  while (txSize + 193 > MAX_TX_SIZE) {
    console.log('inside while loop: big RFQ');
    const halvedInstruments = slicedInstruments.slice(
      0,
      Math.trunc(slicedInstruments.length / 2)
    );
  ...
  let addLegsBuilder: TransactionBuilder | undefined;
  //@ts-ignore
  let addLegsTxSize = 0;

  if (slicedInstruments.length < instruments.length) {
    let addLegsSlicedInstruments = instruments.slice(slicedInstruments.length);
  ...
  //@ts-ignore
  return [rfqBuilder, addLegsBuilder];
};
