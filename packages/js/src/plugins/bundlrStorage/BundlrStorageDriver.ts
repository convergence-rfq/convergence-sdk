...
export class BundlrStorageDriver implements StorageDriver {
  protected _convergence: Convergence;
  protected _bundlr: WebBundlr | NodeBundlr | null = null;
  protected _options: BundlrOptions;

  constructor(convergence: Convergence, options: BundlrOptions = {}) {
    this._convergence = convergence;
    this._options = {
      providerUrl: convergence.connection.rpcEndpoint,
      ...options,
    };
  }
  ...
  async uploadAll(files: ConvergenceFile[]): Promise<string[]> {
    const bundlr = await this.bundlr();
    const amount = await this.getUploadPrice(
      getBytesFromConvergenceFiles(...files)
    );
    await this.fund(amount);

    const promises = files.map(async (file) => {
      const { status, data } = await bundlr.uploader.upload(file.buffer, {
        tags: getConvergenceFileTagsWithContentType(file),
      });
  ...
  async withdrawAll(): Promise<void> {
    // TODO: Replace with "withdrawAll" when available on Bundlr.
    const bundlr = await this.bundlr();
    const balance = await bundlr.getLoadedBalance();
  ...
  async initBundlr(): Promise<WebBundlr | NodeBundlr> {
    ...
    const identity: Signer =
      this._options.identity ?? this._convergence.identity();
...
const getConvergenceFileTagsWithContentType = (
